...
def analyze_and_plot_emotion_averages(folder_path):
                ...
                for user_id, interactions in data.items():
                    ...
                    for interaction in interactions:
                        ...
                        try:
                            ...
                            # Determine if the recognition was successful
    ...
    # Create a DataFrame from all collected raw data
    df = pd.DataFrame(all_data)

    # --- KEY STEP: Calculate Averages ---
    # Group by the specific emotion AND success status, then calculate the mean for each group.
    means_df = df.groupby(['emotion_exp', 'successful'])[['valence', 'arousal']].mean().reset_index()

    success_means_df = means_df[means_df['successful'] == False].copy()
    ...
    # --- PLOT THE AVERAGES ---
    # This section correctly implements your request.
    """ax = sns.scatterplot(
        data=means_df,
        x='valence',
        y='arousal',
        hue='emotion_exp',  # Color by emotion subtype
        style='successful',  # Differentiate marker by success
        palette=emotion_colors,
        markers={True: 'o', False: 'X'},
        s=250,  # Use larger markers for averages
        alpha=0.9,
        edgecolor='black',
        linewidth=1.5
    )"""
    ...
    plt.title('Correct Emotion Recognitions - Virtual', fontsize=20, pad=20)
    ...
    label_positions = {
        ...
        'Pleasant': (9.3, 5.1, 'right', 'center'),
        'Unpleasant': (0.7, 5.1, 'left', 'center'),
        'Fear': (1.5, 7.5, 'center', 'center'),
        'Anger': (1.0, 6.8, 'center', 'center'),
        'Happiness': (8.7, 6.0, 'center', 'center'),
    ...
    # --- COSTRUISCI MANUALMENTE LA LEGENDA ---
    # 1. Emozioni con marker 'o'
    emotion_labels = means_df['emotion_exp'].unique()
    emotion_legend = [
        Line2D([0], [0],
               marker='X',
               color='w',
               label=emotion,
               markerfacecolor=emotion_colors.get(emotion, 'gray'),
               markeredgecolor='black',
               markersize=16)
        for emotion in emotion_labels
    ]

    emotion_labels = list(emotion_colors.keys())  # <-- ordine desiderato
    ...
    # 2. (opzionale) leggenda “Success/Failure” se la riattivi:
    # success_legend = [
    ...
    # 3. Combina le parti della legenda
    custom_legend = emotion_legend  # + success_legend se serve
